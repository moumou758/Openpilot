...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs):
    ...
    if candidate == CAR.LEXUS_ES_TSS2 and Ecu.hybrid not in found_ecus: # candidate in (CAR.LEXUS_ES_TSS2, CAR.TOYOTA_COROLLA_TSS2, CAR.TOYOTA_PRIUS_TSS2, CAR.LEXUS_RX_TSS2) and Ecu.hybrid not in found_ecus:
    ...
    if candidate in TSS2_CAR or ret.enableGasInterceptorDEPRECATED:
      if sp_tss2_long_tune:
        ...
        tune.kiV = [0.31,  0.33,  0.30,  0.21,  0.20,  0.17, 0.10]
